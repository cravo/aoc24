...
const TK_LEFTPAREN = 1
const TK_RIGHTPAREN = 2
const TK_MUL = 3
const TK_NUMBER = 4
const TK_COMMA = 5
const TK_DO = 6
...
function day03_scanToken(input) {
    let token = { type: TK_IGNORED, value: '' }
    let c = input[scannerPos]

    if (c === '(') {
        token.type = TK_LEFTPAREN;
        token.value = '('
        scannerPos++
    }
    else if (c === ')') {
        token.type = TK_RIGHTPAREN;
        token.value = ')'
        scannerPos++
    }
    else if (c === ',') {
        ...
        token.value = scanNumber(input)
    }
    else if (c === 'm') {
        if (input[scannerPos + 1] === 'u' && input[scannerPos + 2] === 'l') {
            token.type = TK_MUL
            token.value = "mul"
            scannerPos += 3
        }
        ...
        if (input.substring(scannerPos, scannerPos + "don't()".length) === "don't()") {
            token.type = TK_DONT
            token.value = "don't"
            scannerPos += 7
        }
        else if (input.substring(scannerPos, scannerPos + "do()".length) === "do()") {
            token.type = TK_DO
            token.value = "do"
            scannerPos += 4
        }
        ...
        // ignore this, we don't know what it is
        token.value = c
        scannerPos++
    }
...
function day03_scanTokens(input) {
    let tokens = []

    while (scannerPos < input.length) {
        let token = day03_scanToken(input)
        tokens.push(token)
    }
...
function day03() {
    ...
    scannerPos = 0
    let tokens = day03_scanTokens(memory)

    let total = 0

    for (let tokenIndex = 0; tokenIndex < tokens.length - 5; ++tokenIndex) {
        let lhs = 0
        let rhs = 0

        if (tokens[tokenIndex].type === TK_MUL) {
            if (tokens[tokenIndex + 1].type === TK_LEFTPAREN) {
                if (tokens[tokenIndex + 2].type === TK_NUMBER) {
                    lhs = tokens[tokenIndex + 2].value

                    if (tokens[tokenIndex + 3].type === TK_COMMA) {
                        if (tokens[tokenIndex + 4].type === TK_NUMBER) {
                            rhs = tokens[tokenIndex + 4].value

                            if (tokens[tokenIndex + 5].type === TK_RIGHTPAREN) {
                                total += lhs * rhs
                            }
                        }
                    }
...
function day03_2() {
    ...
    for (let tokenIndex = 0; tokenIndex < tokens.length - 5; ++tokenIndex) {
        ...
        if (tokens[tokenIndex].type === TK_DO) {
            domul = true
        }
        else if (tokens[tokenIndex].type === TK_DONT) {
            domul = false
        }
